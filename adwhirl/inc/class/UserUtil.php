...
class UserUtil {
  ...
  public static function getUser($email, $password=null) {
    ...
    echo("get user call ...... 3");

    if(empty($aaa)) {
      return null;
    }
    
    echo("get user call ...... 4");

    $user = array();
    foreach(User::$SDBFields as $field => $meta) {
      $user[$field] = isset($aaa[0][$field]) ? $aaa[0][$field] : '';
        echo("</br>user field name ".$field."field = ".$user[$field]);
    }
    $user['id'] = $aaa[0]['id'];
    
    echo("get user call ...... 5 get user");
...
AdWhirl Team";
    ...
    echo("confirm user .... uid = ".$uid);
    UserUtil::confirmUser($uid);
  ...
  public static function confirmUser($uid) {
    ...
    echo("</br>confirm user call .... step1");
    
    if(empty($aaa[0])) {
      return false;
    }

    echo("</br>confirm user call .... step2");
